...
import type { LedenDiff } from "@/lib/leden-diff";
import type { LidCsvRij } from "@/lib/leden-csv";
...
export default function LedenSyncDialoog({ open, onClose, blauwdrukId }: LedenSyncDialoogProps) {
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch("/api/leden-sync/preview", { method: "POST", body: formData });
  ...
  const handleVerwerk = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/leden-sync/verwerk", { method: "POST", body: formData });
      const json = await res.json();
      if (!json.ok) {
