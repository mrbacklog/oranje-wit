...
export default function LedenDashboard({
  ...
  const gefilterd = useMemo(() => {
    ...
    res = [...res].sort((a, b) => {
      ...
      switch (sortKey) {
        case "naam":
          cmp = `${a.achternaam} ${a.roepnaam}`.localeCompare(
            `${b.achternaam} ${b.roepnaam}`
          );
          break;
        case "leeftijd":
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3">
        <input
          ...
          className="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-300 focus:border-orange-400"
        />
        <select
          value={statusFilter}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 text-left text-gray-500 bg-gray-50">
                ...
                <th
                  className="py-2 px-3 font-medium cursor-pointer select-none hover:text-gray-700"
                ...
                <th className="py-2 px-3 font-medium">Wijzig</th>
              </tr>
            </thead>
            <tbody>
                ...
                >
                  ...
                  <td className="py-2 px-3">
                    <span className="inline-flex items-center gap-1.5 text-sm text-gray-600">
                      <span
                        className={`inline-block w-2.5 h-2.5 rounded-full ${retentieDot(speler.retentie?.risico)}`}
                      />
                  ...
                  <td className="py-2 px-3">
                    <select
                      ...
                      onChange={(e) =>
                        wijzigStatus(
                          speler.id,
                          e.target.value as SpelerStatus
                        )
                      }
      ...
      <div className="flex items-center gap-3">
        ...
        {isPending && (
          <p className="text-xs text-gray-400">Opslaan...</p>
        )}
      </div>
    </div>
  );
