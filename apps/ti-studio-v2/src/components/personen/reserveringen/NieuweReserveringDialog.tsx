...
import { maakNieuweReservering } from "@/actions/reservering-actions";
...
export function NieuweReserveringDialog({ onClose }: NieuweReserveringDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await maakNieuweReservering({ titel, geslacht });
      if (result.ok) {
        onClose();
      } else {
        logger.warn("NieuweReserveringDialog: aanmaken mislukt:", result.error);
  ...
  return createPortal(
    <>
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Nieuwe reserveringsspeler aanmaken"
        style={{
          position: "fixed",
          top: "50%",
          left: "50%",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              ...
              <input
                ...
                style={inputStijl}
                placeholder="Bijv. Vrije positie A1"
                required
              />
            </div>
            <div>
