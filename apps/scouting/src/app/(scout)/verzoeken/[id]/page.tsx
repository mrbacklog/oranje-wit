...
import { SpelerContext } from "@/components/speler-context";
...
export default function VerzoekDetailPage() {
  ...
  const laadData = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/verzoeken/${verzoekId}`);
      if (!res.ok) throw new Error("Verzoek niet gevonden");
      const data = await res.json();
      setVerzoek(data.data);

      // Check rol
      const profielRes = await fetch("/api/scout/profiel");
      if (profielRes.ok) {
  ...
  if (!verzoek) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center px-4 text-center">
        <p className="text-sm text-text-muted">Verzoek niet gevonden</p>
        <button
          type="button"
          onClick={() => router.back()}
    ...
    <div className="min-h-screen bg-surface-dark pb-20">
      ...
      <div className="bg-surface-card px-4 pt-4 pb-4 shadow-none">
        <button
          type="button"
          onClick={() => router.back()}
          className="mb-3 inline-flex items-center gap-1 text-sm text-text-muted"
        >
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              <span className="text-xs text-text-muted">{verzoek.seizoen}</span>
            </div>
          </div>
        ...
        <section>
          ...
          <div className="space-y-3">
            {verzoek.spelerIds.map((relCode) => {
              ...
              return (
                <div key={relCode} className="rounded-xl border border-border-subtle bg-surface-card p-4">
                    ...
                    <div className="flex-1">
                      {spelerInfo ? (
                        <SpelerContext
                          ...
                          fotoUrl={`/api/spelers/${relCode}/foto`}
                          compact
                        />
                      ) : (
                      ...
                      {heeftDraft ? (
                        <>
                          <span className="relative flex h-2 w-2">
                            <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-surface-card opacity-75" />
                            <span className="relative inline-flex h-2 w-2 rounded-full bg-surface-card" />
                          </span>
          ...
          <section>
            ...
            <div className="space-y-2">
              {verzoek.toewijzingen.map((t) => (
                <div
                  key={t.id}
                  className="flex items-center justify-between rounded-lg border border-border-subtle bg-surface-card px-3 py-2"
                >
                  <span className="text-sm text-text-secondary">{t.scoutNaam}</span>
                  <span
