    ...
    memoStatusZetten: {
      ...
      execute: async (params: {
        ...
        const memo = await prisma.werkitem.findUnique({
          ...
          select: { id: true, titel: true, beschrijving: true, status: true, prioriteit: true },
        });
        ...
        await prisma.werkitem.update({ where: { id: params.memoId }, data: updateData as any });

        await logDaisyActie({
          sessieId,
          tool: "memoStatusZetten",
          doPayload: { memoId: params.memoId, ...updateData },
          undoPayload: { memoId: params.memoId, status: memo.status, prioriteit: memo.prioriteit },
          namens: gebruikerEmail,
          uitgevoerdIn: "kanban",
        });
