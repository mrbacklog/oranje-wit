...
import { XPBar } from "@/components/scouting/xp-bar";
import { BadgeGrid } from "@/components/scouting/badge-grid";
import { Leaderboard } from "@/components/scouting/leaderboard";
...
export default function ProfielPage() {
  ...
  const fetchProfiel = useCallback(async () => {
    try {
      const res = await fetch("/api/scouting/scout/profiel");
    ...
    <div className="flex flex-col gap-4 px-4 pt-6 pb-24">
      ...
      <section className="pt-4">
        <button
          ...
          className="bg-surface-card hover:bg-surface-dark w-full rounded-xl border border-neutral-200 px-4 py-3 text-sm font-medium text-neutral-500 transition-colors active:scale-[0.98]"
        >
          Uitloggen
        </button>
      </section>
