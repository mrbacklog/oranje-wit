...
export default function RondeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/evaluatie/rondes/${id}`).then((r) => r.json()),
      fetch(`/api/evaluatie/rondes/${id}/teams`).then((r) => r.json()),
  ...
  async function updateStatus(status: string) {
    setActie("status");
    await fetch(`/api/evaluatie/rondes/${id}`, {
      method: "PATCH",
  ...
  async function verstuurHerinneringen() {
    setActie("herinneren");
    const res = await fetch(`/api/evaluatie/rondes/${id}/herinneren`, {
      method: "POST",
    });
