...
export default function UitgangspositiePanel({
  ...
  const handleToggleTeam = useCallback(
    async (teamId: string) => {
      ...
      if (!spelerCache[teamId]) {
        ...
        try {
          const res = await fetch(`/api/teamindeling/referentie-teams/${teamId}/spelers`);
          if (res.ok) {
  ...
  const handleScoreChange = useCallback(
    (teamId: string, value: string) => {
      ...
      debounceRef.current[teamId] = setTimeout(async () => {
        ...
        setSaving(teamId);
        try {
          const res = await fetch(`/api/teamindeling/referentie-teams/${teamId}/teamscore`, {
            method: "PATCH",
  ...
  const handleHerbereken = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/teamindeling/ratings/herbereken", { method: "POST" });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleVervresBevestig = useCallback(
    async (keuzes: Record<string, "behoud" | "reset">) => {
      try {
        ...
        const res = await fetch("/api/teamindeling/referentie-teams/ververs", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
