...
export function VerzoekKaart({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="border-border-subtle bg-surface-card w-full rounded-xl border p-4 text-left transition-shadow hover:shadow-md active:scale-[0.98]"
    >
      {/* Bovenste rij: badges */}
      ...
      <div className="mt-2">
        ...
        {toelichting && (
          <p className="text-text-muted mt-0.5 line-clamp-2 text-xs">{toelichting}</p>
        )}
      </div>
