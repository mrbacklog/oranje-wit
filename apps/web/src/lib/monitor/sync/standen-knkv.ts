...
async function isStale(seizoen: string): Promise<boolean> {
  ...
  const seizoenExists = await (prisma.seizoen.findUnique as Function)({
    where: { seizoen },
    select: { seizoen: true },
  });
  if (!seizoenExists) return false;

  const result = await (prisma.poolStand.aggregate as Function)({
    where: { seizoen },
  ...
  for (const { sport, periode } of SPORT_PERIODES) {
    ...
    for (const [refId, poolNaam] of poolMap) {
      try {
        ...
        // Upsert pool_standen
        const poolStand = await (prisma.poolStand.upsert as Function)({
          where: {
        ...
        await (prisma.poolStandRegel.deleteMany as Function)({
          where: { poolStandId: poolStand.id },
        });

        await (prisma.poolStandRegel.createMany as Function)({
