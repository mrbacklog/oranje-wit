...
const leesTools = {
  spelersZoeken: {
    ...
    execute: async (params: {
      ...
      // Team/kleur-filter op DB-niveau via JSON path
      if (params.kleur) {
      ...
      const spelers = await prisma.speler.findMany({
        ...
        take: params.team || params.kleur ? 30 : 50,
      });
  ...
  competitieTeamZoeken: {
    ...
    execute: async (params: {
      ...
      // 1. Zoek ow_team_id via team_aliases (exact of case-insensitive)
      ...
      // 2. Haal spelers op (dedup op rel_code: één speler één keer, ook al staat die in veld+zaal)
      ...
      // 3. Haal staf op (alleen als metStaf)
