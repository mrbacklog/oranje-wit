// @ts-nocheck — NextResponse type conflict door meerdere Next.js installaties
...
export function ok<T>(data: T, options?: OkOptions): NextResponse<ApiResponse<T>> {
  return baseOk(data, { cacheControl: "no-store", ...options }) as NextResponse<ApiResponse<T>>;
}
