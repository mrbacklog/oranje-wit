...
async function main() {
  ...
  const { rows: leden } = await client.query('SELECT rel_code, roepnaam, tussenvoegsel, achternaam FROM leden');
  ...
  const skipPatterns = [/^vacature/i, /^n\.n\.b/i, /^nog in te vullen/i, /^eigen$/i,
    /^carousel/i, /^carrousel/i, /^trainerscarrousel/i, /^teamlft/i, /^s1s2/i,
    /^coach vacature/i, /^demi$/i, /^\d+ speelt/i, /^1x in de week/i];

  const norm = s => s.toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '') // accenten verwijderen
    .replace(/[^a-z ]/g, '')
    .replace(/\s+/g, ' ')
    .trim();
  ...
  for (const naam of stafNamen) {
    ...
    if (result) {
      ...
      const tv = lid.tussenvoegsel ? lid.tussenvoegsel + ' ' : '';
      matched.push({ stafNaam: naam, relCode: lid.rel_code, lidNaam: lid.roepnaam + ' ' + tv + lid.achternaam, method: result.method });
      continue;
    }
    ...
    if (naam.includes(',')) {
      ...
      if (isInitiaal) {
        ...
        if (initiaalMatch) {
          ...
          const tvInAchternaam = achternaamPart.match(/^(de|van|van de|van den|van der|ter|te)\s+/i);
          if (tvInAchternaam) {
          ...
          if (candidates.length === 1) {
            ...
            const tv = lid.tussenvoegsel ? lid.tussenvoegsel + ' ' : '';
            matched.push({ stafNaam: naam, relCode: lid.rel_code, lidNaam: lid.roepnaam + ' ' + tv + lid.achternaam, method: 'initiaal' });
            found = true;
          } else if (candidates.length > 1) {
            notMatched.push(naam + ' [AMBIG: ' + candidates.map(c => c.roepnaam + ' ' + c.achternaam).join(' / ') + ']');
            found = true;
          }
        }
      }
  ...
  const echteNotMatched = notMatched.filter(n => !n.includes('[GEEN LID]') && !n.includes('[AMBIG'));
  ...
  console.log('Gecategoriseerd:', ((matched.length + geenLid + ambig) / personen * 100).toFixed(1) + '%');
  console.log();
  ...
  notMatched.filter(n => n.includes('[GEEN LID]')).forEach(n => console.log('  ' + n.replace(' [GEEN LID]', '')));
  console.log();
