...
const SKIP_PATTERNS = [/^vacature/i, /^n\.n\.b/i, /^nog in te vullen/i, /^eigen$/i,
...
function isSkippable(naam) {
  ...
  if (naam.includes('/') && naam.split('/').length === 2 && naam.split('/')[1].trim().length > 3) return true;
  return false;
}
...
async function main() {
  ...
  console.log(`Seizoenen: ${seizoenen.length} (${seizoenen[0]} t/m ${seizoenen[seizoenen.length - 1]})`);

  if (DRY_RUN) {
    console.log('\n=== DRY RUN — geen database-wijzigingen ===');
    console.log('\nEerste 10 personen:');
    deduped.slice(0, 10).forEach(p =>
      console.log(`  ${p.stafCode} — ${p.naam} (relCode: ${p.relCode || '-'})`)
    );
    console.log('\nEerste 10 toewijzingen:');
    toewijzingen.slice(0, 10).forEach(t =>
      console.log(`  ${t.stafId} — ${t.seizoen} — ${t.team} — ${t.rol}`)
    );
    await client.end();
    return;
  }
  ...
  // 8. Verifieer
  const { rows: [{ count: stafCount }] } = await client.query('SELECT COUNT(*) as count FROM "Staf"');
  const { rows: [{ count: twCount }] } = await client.query('SELECT COUNT(*) as count FROM staf_toewijzingen');
