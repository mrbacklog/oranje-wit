...
const RELEVANTE_SEIZOENEN = ["2021-2022", "2022-2023", "2023-2024", "2024-2025", "2025-2026"];
...
function extractRetentieUitYAML(yamlTekst) {
  ...
  for (const regel of yamlTekst.split("\n")) {
    ...
    if (trimmed.startsWith("per_leeftijd_M:")) {
      huidigeSectie = "per_leeftijd_M";
      continue;
    }
    if (trimmed.startsWith("per_leeftijd_V:")) {
      huidigeSectie = "per_leeftijd_V";
      continue;
    }
    if (trimmed.startsWith("per_leeftijd:")) {
      huidigeSectie = "per_leeftijd";
      continue;
    }
    if (trimmed.startsWith("samenvatting:") || trimmed.startsWith("vergelijking")) {
      huidigeSectie = null;
      continue;
    }
...
function extractInstroomUitYAML(yamlTekst) {
  ...
  for (const regel of yamlTekst.split("\n")) {
    const trimmed = regel.trim();
    if (trimmed.startsWith("verdeling_per_leeftijd:")) {
      inSectie = true;
      continue;
    }
    if (inSectie && trimmed.startsWith("gemiddeld_per_seizoen:")) {
      inSectie = false;
      continue;
    }
...
function zoekTeamInfo(teamsPerSeizoen, seizoen, teamNaam) {
  ...
  if (match) {
    return {
      ...
      spelvorm:
        match.spelvorm ||
        (match.kleur && ["Blauw", "Groen"].includes(match.kleur) ? "4-tal" : "8-tal"),
    };
  }
...
function bepaalACategorieVolgendSeizoen(geboortejaar) {
  ...
  if (leeftijd >= 13 && leeftijd <= 14)
    return { a_categorie: "U15", a_jaars: leeftijd === 13 ? "1e-jaars" : "2e-jaars" };
  if (leeftijd >= 15 && leeftijd <= 16)
    return { a_categorie: "U17", a_jaars: leeftijd === 15 ? "1e-jaars" : "2e-jaars" };
  if (leeftijd >= 17 && leeftijd <= 18)
    return { a_categorie: "U19", a_jaars: leeftijd === 17 ? "1e-jaars" : "2e-jaars" };
...
function bepaalVolgendSeizoen(speler) {
  ...
  if (
    speler.huidig &&
    speler.huidig.a_categorie &&
    aCat.a_categorie !== speler.huidig.a_categorie
  ) {
...
function berekenRetentie(speler, retentieData, seizoenenActief) {
  ...
  let kansBehoud =
    retentieData[geslachtKey]?.[leeftijd] || retentieData.per_leeftijd?.[leeftijd] || 0.9;
...
function bouwStreefmodel(streefData) {
  ...
  for (const [bandNaam, bandInfo] of Object.entries(banden)) {
    ...
    perBand.push({
      ...
      vulgraad:
        huidigBand.totaal && streefBand.totaal
  ...
  for (const cat of ["U15", "U17", "U19"]) {
    ...
    const poolSpelers = boogHuidig
      ...
      .reduce((sum, p) => ({ totaal: sum.totaal + p.totaal, m: sum.m + p.m, v: sum.v + p.v }), {
        totaal: 0,
        m: 0,
        v: 0,
      });
...
function bepaalACategorieGeboortejaren(cat) {
  switch (cat) {
    case "U15":
      return [seizoenJaar - 13, seizoenJaar - 14];
    case "U17":
      return [seizoenJaar - 15, seizoenJaar - 16];
    case "U19":
      return [seizoenJaar - 17, seizoenJaar - 18];
    default:
      return [];
  }
}
...
function berekenVulgraadStatus(huidig, streef) {
  ...
  if (ratio >= 0.8) return "op_koers";
  if (ratio >= 0.6) return "aandacht";
  return "kritiek";
}
...
async function main() {
  ...
  console.log(
    `   Teams-KNKV beschikbaar voor: ${[...teamsPerSeizoen.keys()].join(", ") || "geen"}`
  );
  ...
  const teamsHuidig = teamStats.map((team) => {
    ...
    return {
      ...
      spelvorm:
        knkvTeam?.spelvorm ||
        (knkvTeam?.kleur && ["Blauw", "Groen"].includes(knkvTeam.kleur) ? "4-tal" : "8-tal"),
  ...
  const retentieExport = {
    ...
    risico_drempels: { laag: 0.9, gemiddeld: 0.85, hoog: 0.8 },
  };
  ...
  console.log(
    `  Verloop:      ${Object.keys(exportData.verloop.samenvatting).length ? "OK" : "ontbreekt"}`
  );
