...
async function slaStandOp(client, seizoen, periode, poolNaam, standings, standDatum) {
  ...
  if (standings.length > 0) {
    ...
    for (const entry of standings) {
      valuesStr.push(`($${idx}, $${idx+1}, $${idx+2}, $${idx+3}, $${idx+4}, $${idx+5}, $${idx+6}, $${idx+7}, $${idx+8}, $${idx+9}, $${idx+10})`);
      idx += 11;
    ...
    for (const { sport, periode } of SPORT_PERIODES) {
      ...
      for (const [refId, poolNaam] of poolMap) {
        try {
          ...
          const aantalRegels = await slaStandOp(client, SEIZOEN, periode, poolNaam, standings, standDatum);
