/**
 * Seed vaardigheidsraamwerk naar de database.
 *
 * Maakt een RaamwerkVersie aan voor seizoen 2025-2026 met status ACTIEF,
 * vult alle banden (blauw t/m rood) met hun pijlers en ontwikkelitems.
 *
...
async function main() {
  process.stdout.write(`Seed raamwerk voor seizoen ${SEIZOEN}...\n\n`);
  ...
  for (const groepDef of GROEPEN) {
    ...
    const groep = await prisma.leeftijdsgroep.upsert({
      where: {
        versieId_band: {
          versieId: versie.id,
          band: groepDef.band,
    ...
    for (let pIdx = 0; pIdx < groepDef.pijlers.length; pIdx++) {
      ...
      const pijler = await prisma.pijler.upsert({
        where: {
          groepId_code: {
            groepId: groep.id,
      ...
      for (let iIdx = 0; iIdx < pijlerDef.items.length; iIdx++) {
        ...
        await prisma.ontwikkelItem.upsert({
          where: {
            pijlerId_itemCode: {
              pijlerId: pijler.id,
